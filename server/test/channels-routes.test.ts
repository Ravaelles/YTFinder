...
describe("channel routes", () => {
  ...
  it("refresh adds only missing videos and preserves existing", async () => {
    prismaMock.channel.findUnique.mockResolvedValue({ id: 3, sourceUrl: "https://www.youtube.com/@Yoga/videos" });
    scanChannelMock.mockResolvedValue({
      handle: "@Yoga",
      displayName: "Yoga",
      youtubeChannelId: "channel-1",
      ...
      videos: [
        {
          youtubeVideoId: "existing",
          title: "Old",
          durationSec: 120,
          publishedAt: new Date("2025-01-01T00:00:00.000Z"),
          viewCount: 10,
          videoUrl: "https://www.youtube.com/watch?v=existing"
        },
        {
          youtubeVideoId: "new-one",
          title: "New",
          durationSec: 180,
          publishedAt: new Date("2025-01-02T00:00:00.000Z"),
          viewCount: 11,
          videoUrl: "https://www.youtube.com/watch?v=new-one"
        }
      ]
    });
    prismaMock.video.findMany.mockResolvedValue([{ youtubeVideoId: "existing" }]);
    prismaMock.video.createMany.mockResolvedValue({ count: 1 });
    prismaMock.channel.update.mockResolvedValue({ id: 3 });

    const app = buildApp();
    const res = await app.inject({ method: "POST", url: "/api/channels/3/refresh" });

    expect(res.statusCode).toBe(200);
    expect(prismaMock.video.createMany).toHaveBeenCalledTimes(1);
    expect(prismaMock.video.createMany.mock.calls[0][0].data).toHaveLength(1);
    expect(prismaMock.video.createMany.mock.calls[0][0].data[0].youtubeVideoId).toBe("new-one");
    expect(res.json()).toEqual({ addedCount: 1 });
    await app.close();
  });
});
